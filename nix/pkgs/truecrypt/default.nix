...
{ fetchurl, stdenv, pkgconfig, nasm, fuse, wxGTK, devicemapper, makeWrapper
, wxGUI ? true }:

stdenv.mkDerivation {
  name = "truecrypt-7.1a";

  buildInputs = [ pkgconfig fuse devicemapper wxGTK nasm makeWrapper ];
  ...
  configurePhase = ''
    ...
    mkdir wx
    tar -xjf "${wxGTK.src}" -C wx --strip-components 1
    patchShebangs wx
  '';

  buildPhase = ''
    make $makeFlags WX_ROOT=$PWD/wx wxbuild
    make $makeFlags WXSTATIC=1 PKCS11_INC="`pwd`"
  '';
